...
import { syncPendingCart } from "../../cart/servers/cart.action";
import { syncPendingWishlist } from "../../WishList/servers/wishlist.action";
...
export default async function loginAction(values: LoginFormValues) {
  ...
  try {
    ...
    if (data.message === "success") {
      ...
      // Sync guest cart and wishlist while token is already in cookie store
      await Promise.allSettled([syncPendingCart(), syncPendingWishlist()]);
